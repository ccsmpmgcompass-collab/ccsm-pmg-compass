...
| 6 | Streamlit Community Cloud account | Owns the deployed app — **https://ccsm-pmg-compass-dvqpedw6bqixxscth8zapn.streamlit.app/** (recorded 2026-09-08; it was an ellipsis here and written down nowhere else, so it cost a search every time it was needed) — Settings > Secrets, Reboot, Logs, and the Share/invite list (a **separate** allowlist from the code-level one in `auth.py`) | Cloud account sign-in only | No — human confirms by logging in and checking Manage app |
