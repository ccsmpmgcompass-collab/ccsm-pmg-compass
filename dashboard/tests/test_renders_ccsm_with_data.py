...
def _clear_query_caches() -> None:
    ...
    defined in the PAGE modules (pages/06_Scores.py::_load_daily), not in
...
PAGES = ["pages/01_dashboard.py", "pages/06_Scores.py"]
...
def test_dashboard_shows_ccsms_key_indicators():
    ...
    body = _text(_run("pages/01_dashboard.py"))
...
def test_daily_activity_totals_every_nightly_metric():
    ...
    Searching the whole body passes for the wrong reason: 06_Scores.py's
