...
# ── The verdict banner (audit item 8) ─────────────────────────────────────────

_ACCENT = "linear-gradient(180deg,#6366f1,#8b5cf6)"


def _banner(at) -> str:
    """The verdict banner's markup, or "" if the page did not draw one."""
    return next((str(m.value) for m in at.markdown
                 if "informes:" in str(m.value)), "")


def _banner_parts(at) -> tuple[str, str]:
    """(context line, verdict line) as plain text."""
    import re
    divs = re.findall(r">([^<>]+)<", _banner(at))
    body = [d.strip() for d in divs if d.strip()]
    return (body[0], body[1]) if len(body) >= 2 else ("", "")


def test_the_panel_opens_with_a_verdict_banner():
    """Nine sections of numbers, and until item 8 nothing on the page said what
    any of them added up to. The banner must come BEFORE the first section, or
    it is a summary the reader reaches after already reading everything."""
    at = _run("pages/01_Panel.py")
    banner = _banner(at)
    assert banner, "the Panel drew no verdict banner"
    assert _ACCENT in banner, "the banner is missing its accent bar"

    order = [i for i, m in enumerate(at.markdown)
             if "informes:" in str(m.value)
             or "ACTIVIDAD DIARIA" in str(m.value).upper()]
    assert order, "neither the banner nor section 1 rendered"
    first = str(at.markdown[order[0]].value)
    assert "informes:" in first, "section 1 rendered above the verdict banner"


def test_the_verdict_omits_the_weekly_share_until_a_week_is_complete():
    """This fixture's newest WEEKLY_KI week is still the one in progress, so
    only the nightly denominator may be shown. A percentage of a half-finished
    week reads as a failure that has not happened yet — the same rule section 2
    already follows with "have reported so far"."""
    context, _ = _banner_parts(_run("pages/01_Panel.py"))
    assert "nocturno" in context
    assert "semanal" not in context


def test_the_verdict_never_names_a_rate_it_cannot_read():
    """friend_lessons is absent from this fixture, so two of the four rates
    divide by nothing. worst_rate skips them; the sentence must not name one as
    the mission's weakest point, because an unmeasured rate is not a bad one."""
    _, verdict = _banner_parts(_run("pages/01_Panel.py"))
    assert verdict, "the banner drew no verdict line"
    assert "Invitación Bautismal" not in verdict
    assert "Lecciones" not in verdict
    # Every readable rate clears its target here, so the sentence must say so
    # rather than invent a weakest point.
    assert "en meta" in verdict


def test_the_verdict_names_the_weakest_rate_when_all_four_are_readable():
    """With a denominator for all four, the sentence becomes the one the page
    normally shows: the single rate furthest below its target. 60 lessons
    against 5 baptismal invitations is a close_rate of 8,3% on a 25% target —
    far below contact_rate and mc_rate, which sit above theirs."""
    dl = _TABS["DAILY_LOG"].copy()
    dl["friend_lessons"] = "60"
    dl["baptismal_invitations"] = "5"
    _TABS["DAILY_LOG"] = dl

    _, verdict = _banner_parts(_run("pages/01_Panel.py"))
    assert "Invitación Bautismal" in verdict, verdict
    assert "8,3%" in verdict and "25%" in verdict, verdict
