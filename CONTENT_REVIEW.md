# CONTENT_REVIEW — MESSAGE_BANK + KNOWLEDGE_BASE (CCSM)

**Launch gate.** Every missionary-facing word the CCSM system can send is listed below.
Message selection is *pick-not-generate*: the agents choose one of these rows by ID and send it verbatim.
Nothing here is AI-generated at send time, so this file is the complete vocabulary of the system.
...
Generated from `CCSM_SeedContent.gs` — do not edit this file by hand. Edit the `.gs` and regenerate with
`node tools/gen_content_review.js`.
...
### 1. Scripture_Text is BLANK on 191 of 193 rows — this is the most important review task
...
### 2. The two rows that DO carry Scripture_Text came verbatim from the task brief — please verify them

- **MSG-CS-ROLEPLAYS-01** — `D. y C. 84:85`: "Atesorad continuamente en vuestra mente las palabras de vida."
- **MSG-CG-CONTACTS-01** — `D. y C. 33:8-10`: "Abrid vuestra boca, y será llena."

These two rows also carry the only Predicad Mi Evangelio **page numbers** in the whole bank
(`pág. 175`, `pág. 156`). Those page numbers came from the brief and refer to the English edition —
the Spanish edition paginates differently, so they are very likely wrong for Chile. Recommend either
correcting them to the Spanish pagination or dropping them to chapter level like every other row.

### 3. Predicad Mi Evangelio references are chapter-level, never page-level

Every other row cites `Capítulo N` plus a one-line topic summary written for this system (not a quoted
...
<sub>Predicad Mi Evangelio, 175 — Predicad Mi Evangelio, pág. 175 — La práctica mejora la enseñanza · D. y C. 84:85: "Atesorad continuamente en vuestra mente las palabras de vida."</sub>
...
<sub>Predicad Mi Evangelio, 156 — Predicad Mi Evangelio, pág. 156 — Hablar con todos · D. y C. 33:8-10: "Abrid vuestra boca, y será llena."</sub>
