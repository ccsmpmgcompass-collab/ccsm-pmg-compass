...
**Launch gate.** The two seeded content banks are listed below, plus the leadership
messages from the `LEADERSHIP_MESSAGE_BANK` sheet tab.
...
Generated from `CCSM_SeedContent.gs` and the `LEADERSHIP_MESSAGE_BANK` sheet tab — do not edit this file by hand.
Edit the `.gs` (or the sheet tab) and regenerate with `node tools/gen_content_review.js`.
...
- **`LEADERSHIP_MESSAGE_BANK` (Task 8).** The leadership message bank was trimmed pending this content review.
...
## MENSAJES DE LIDERAZGO — `LEADERSHIP_MESSAGE_BANK` (Sheet tab)
